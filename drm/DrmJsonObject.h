...
*/
#ifndef _DRM_JSON_OBJECT_H
#define _DRM_JSON_OBJECT_H

/**
 * @file DrmJsonObject.h
...
public:
	DrmJsonObject();
	DrmJsonObject(const std::string& jsonStr);
	DrmJsonObject(const char* jsonStr);
	~DrmJsonObject();
	DrmJsonObject(const DrmJsonObject&) = delete;
	DrmJsonObject& operator=(const DrmJsonObject&) = delete;
	DrmJsonObject(DrmJsonObject &&) = default;
	 ...
	 *
	 *  @param name name for the array
	 *  @param values vector of #DrmJsonObject to add as an array
	 *  @return true if successfully added, false otherwise
	 */
	bool add(const std::string& name, std::vector<DrmJsonObject*>& values);
	 ...
	 * @return true if successfully retrieved and decoded value, false otherwise
	 */
	bool get(const std::string& name, DrmJsonObject &value);
	 ...
	 *  @return true if successfully retrieved, false otherwise
	 */
	bool get(const std::string& name, std::vector<DrmJsonObject> &values);
	...
	bool add(const std::string& name, DrmJsonObject& value);
...
private:
	bool set(DrmJsonObject *parent, cJSON *object);
	bool add(const std::string& name);
	cJSON *mJsonObj;
	DrmJsonObject *mParent;
};

/**
 * @class DrmJsonParseException
 * @brief Handles the exception for JSON parser
 */
class DrmJsonParseException : public std::exception
{
public:
